@echo off
rem ---------------------------------------------------------------------------
rem  Builds Emerald and opens the Ingest Controller.
rem    run-ingest.bat              - Release build, then run
rem    run-ingest.bat Debug        - Debug build, then run
rem    run-ingest.bat Release -n   - skip the build, just run what is already built
rem
rem  Emerald runs once. If it is already up - the deck, the EDL, or another
rem  module - this hands the request over and that instance opens the Ingest
...
set "ROOT=%~dp0"
set "MODULE=--ingest"
set "MODULE_NAME=Ingest Controller"
