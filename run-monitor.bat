@echo off
rem ---------------------------------------------------------------------------
rem  Builds Emerald and opens the Monitor - everything from capture to on air.
rem    run-monitor.bat              - Release build, then run
rem    run-monitor.bat Debug        - Debug build, then run
rem    run-monitor.bat Release -n   - skip the build, just run what is already built
rem
rem  Emerald runs once. If it is already up - the deck, the EDL, or another
rem  module - this hands the request over and that instance opens the Monitor
rem  Controller alongside whatever is already on screen. Nothing is closed, and
rem  the build is skipped, because the running instance holds its own exe.
rem
...
rem instance that will open the window, so there is nothing to rebuild for.
set "RUNNING=0"
tasklist /fi "imagename eq Emerald.App.exe" /nh 2>nul | "%SystemRoot%\System32\find.exe" /i "Emerald.App.exe" >nul && set "RUNNING=1"

if "%RUNNING%"=="1" (
    echo Emerald is already running - opening the %MODULE_NAME% in it.
) else if /i "%~2"=="-n" (
    echo Skipping build.
) else (
...
echo Launching %MODULE_NAME% ...
start "" "%EXE%" %MODULE%
exit /b 0

:fail
echo.
echo [ERROR] Could not start the %MODULE_NAME%.
