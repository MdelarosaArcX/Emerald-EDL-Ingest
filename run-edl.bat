@echo off
rem ---------------------------------------------------------------------------
rem  Builds Emerald and opens the EDL Generator.
rem    run-edl.bat              - Release build, then run
rem    run-edl.bat Debug        - Debug build, then run
rem    run-edl.bat Release -n   - skip the build, just run what is already built
rem
rem  Emerald runs once. If it is already up - the deck, the EDL, or another
rem  module - this hands the request over and that instance opens the EDL
rem  Generator alongside whatever is already on screen. Nothing is closed, and
